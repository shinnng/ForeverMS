...
func LoginReturnFromChannel() maplepacket.Packet {
	...
	pac.WriteByte(maplepacket.SendLoginRestarter)
	pac.WriteByte(0x01)
...
func LoginResponce(result byte, userID int32, gender byte, isAdmin byte, username string, isBanned int) maplepacket.Packet {
	pac := maplepacket.NewPacket()
	pac.WriteByte(maplepacket.SendLoginResponce)
...
func LoginMigrateClient(ip []byte, port int16, charID int32) maplepacket.Packet {
	pac := maplepacket.NewPacket()
	pac.WriteByte(maplepacket.SendLoginCharacterMigrate)
	pac.WriteByte(0x00)
	pac.WriteByte(0x00)
	pac.WriteBytes(ip)
	pac.WriteInt16(port)
...
func LoginDisplayCharacters(characters []*models.Character) maplepacket.Packet {
	pac := maplepacket.NewPacket()
	pac.WriteByte(maplepacket.SendLoginCharacterData)
	pac.WriteByte(0) // ?
...
func LoginNameCheck(name string, nameFound int) maplepacket.Packet {
	pac := maplepacket.NewPacket()
	pac.WriteByte(maplepacket.SendLoginNameCheckResult)
...
func LoginCreatedCharacter(success bool, character *models.Character) maplepacket.Packet {
	pac := maplepacket.NewPacket()
	pac.WriteByte(maplepacket.SendLoginNewCharacterGood)
...
func LoginDeleteCharacter(charID int32, deleted bool, hacking bool) maplepacket.Packet {
	pac := maplepacket.NewPacket()
	pac.WriteByte(maplepacket.SendLoginDeleteCharacter)
	pac.WriteInt32(charID)
...
func LoginWorldListing(worldIndex byte) maplepacket.Packet {
	pac := maplepacket.NewPacket()
	pac.WriteByte(maplepacket.SendLoginWorldList)
	pac.WriteByte(worldIndex)                      // world id
	pac.WriteString(consts.WorldNames[worldIndex]) // World name -
	...
	for j := 1; j < 21; j++ {
		pac.WriteString(consts.WorldNames[worldIndex] + "-" + strconv.Itoa(j))         // channel name
...
func LoginWorldInfo(warning byte, population byte) maplepacket.Packet {
	...
	p.WriteByte(maplepacket.SendLoginWorldMeta)
